...
function ProfilePage() {
    ...
    const handleImageChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                ...
                setFiles(prev => ({ ...prev, photo: compressed }));
            } catch (err) {
    ...
    const onSubmit = async (data) => {
        ...
        if (!userId) {
            dispatch(showToast('User session expired. Please login again.', 'error'));
            navigate('/login');
            return;
        }
        ...
        try {
            ...
            // Pass photo separately to match the API service signature
            const response = await authAPI.updateUserProfile(userId, profileData, { photo: files.photo });
                    ...
                    <AvatarWrapper>
                        {avatar ? (
                            ...
                            <div style={{
                                ...
                                <MdOutlineAccountCircle size={80} color="var(--card-background)" />
                            </div>
                        )}
